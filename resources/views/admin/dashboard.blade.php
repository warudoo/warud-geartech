...
@section('content')
    <section class="mb-6">
        <div>
            <p class="eyebrow font-extrabold">Admin Console</p>
            <h1 class="page-title">Dashboard Operasional</h1>
            <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
                Monitoring order terbaru, kondisi stok, dan ringkasan performa toko dari satu workspace yang lebih rapi
                untuk
                operasional harian Warud Geartech.
            </p>
        </div>
    </section>
    ...
    <section class="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="admin-stat-card">
            ...
            <p class="admin-stat-value">{{ $activeProducts }} <span class="text-lg text-slate-400">/
                    {{ $totalProducts }}</span></p>
            ...
            <div class="space-y-3">
                @forelse($recentOrders as $order)
                    <a href="{{ route('admin.orders.show', $order) }}"
                        class="admin-list-card flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-3 lg:text-right">
                            <span
                                class="rounded-full px-3 py-1 text-xs uppercase tracking-[0.24em] ring-1 {{ $order->status->badgeClasses() }}">
                                {{ $order->status->label() }}
                            </span>
                ...
                <div class="space-y-3">
                    @forelse($lowStockProducts as $product)
                        <a href="{{ route('admin.products.edit', $product) }}"
                            class="admin-list-card flex items-center justify-between gap-4">
                            <div>
                                <p class="font-semibold text-slate-900">{{ $product->name }}</p>
                                <p class="mt-1 text-sm text-slate-600">{{ $product->category->name }} /
                                    {{ $product->brand }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs uppercase tracking-[0.18em] text-slate-500">Stock</p>
                                <p
                                    class="font-display text-2xl {{ $product->stock <= 2 ? 'text-rose-600' : 'text-amber-600' }}">
                                    {{ $product->stock }}</p>
                            </div>
                        </a>
                    @empty
