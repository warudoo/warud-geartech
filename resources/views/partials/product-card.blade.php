<article class="group panel overflow-hidden">
    <div class="relative aspect-[5/4] overflow-hidden rounded-2xl border border-slate-200 bg-slate-100">
        @if($product->featured_image || $product->image_url)
            <img src="{{ $product->featured_image ?: $product->image_url }}" alt="{{ $product->name }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105">
        @else
            <div class="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(220,38,38,0.16),_transparent_55%),linear-gradient(180deg,_rgba(255,255,255,0.85),_rgba(241,245,249,1))]"></div>
        @endif
