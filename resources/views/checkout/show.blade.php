            ...
            <div class="space-y-4">
                @foreach ($cartItems as $item)
                    <div class="panel-muted flex items-center justify-between gap-4 p-4">
                        <div>
                            <p class="text-sm uppercase tracking-[0.18em] text-slate-500">{{ $item->product->brand }} •
                                {{ $item->product->sku }}</p>
                            <p class="mt-1 font-semibold text-slate-900">{{ $item->product->name }}</p>
                            <p class="mt-1 text-sm text-slate-600">{{ $item->quantity }} x Rp
                                {{ number_format((float) $item->product->price, 0, ',', '.') }}</p>
                        </div>
                        <p class="font-display text-xl text-red-600">Rp {{ number_format($item->lineTotal(), 0, ',', '.') }}
                        </p>
                    </div>
                @endforeach
            </div>
            ...
            <form method="POST" action="{{ route('checkout.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input id="customer_name" type="text" name="customer_name"
                        value="{{ old('customer_name', $user->name) }}" class="form-input" required>
                </div>
